...
## Angarita H., Yates D., Depsky N. 2017
## This library constains some functions to pertorm the following tasks:
## - Read data of GCM models from NetCDF files in a bounding box
## - Compare GCm results of a given point to the local meteorological time series
## - Perform a kNN-Boostrapping
...
#----------------------------------------------------------------------
# *** this library contains all the functions of the GCM Clim Tool ***
#----------------------------------------------------------------------
...
tri_state_joint_prob_monthly = function(timeSeriesdf, lowerBounds, higherBounds, monthly_thresholds = TRUE){ # requires a time series object input df with 3 columns --> year, month, value, and vectors of monthly lower and upper bounds, monthly_thresholds dictates whether or not to use monthly changing lower and upper bound thresholds, if FALSE lowerBounds and higherBounds are a single constant value 
  ...
  for(m in 1:12)
  {
    ...
    jp <- as.data.frame(jp)
    row.names(jp) <- c("fromLow", "fromMid", "fromHigh")
    colnames(jp) <- as.vector(sapply(month.abb, function(x) paste0(c("toLow", "toMid", "toHigh"), x)))
  }
...
#----------------------------------------------------------------------------
# II. FUNCTION TO READ GCM AND OBSERVATIONS AND PRODUCE CORRESPONDING DATA SETS
#----------------------------------------------------------------------------
read_GCM_NCDF = function(boundingBox = c(latMin = 1,latMax = 3,lonMin = -76,lonMax = -74),                         #refDate = c(month = 1, day = 1, year = 2006)    # RCP future Simulations
                         ...
                         GCM_folders = c("/GCMDailyPrcp/CESM/RCP8.5-E1","/GCMDailyPrcp/CESM/RCP8.5-E2"),   # this path is relative to the main_directory. Default values are just for illustration purpose
                         variableName = "tas",
                         variableLabel = "Temperature [k]",
                         shp_file = "/GIS/WEAP_CATCHMENTS_V14_61_WGS84",                                   # this path is relative to the main_directory
  ...
  fFolders = paste(main_folder,GCM_folders,sep="")
  
  # IMPORTANT: SHP FILE with the boundary of the basin:
  ...
  if (boundingBox[3] < 0){
    lonMin = boundingBox[3] + 360
  } else {
    lonMin = boundingBox[3]
  }
  
  if (boundingBox[4] < 0){
    lonMax = boundingBox[4] + 360
  } else {
  ...
  for (fPath in fFolders) {
    
    # Gets a lists of the netCDF files in the folder.
    fList = list.files(path = fPath,pattern = "*.nc", full.names = TRUE)
    fList
    ...
    # loops through the file lists
    
    for (fName in fList) {
        
        # Reads the NetCDF file
        fName
        control = open.nc(con=fName, write=FALSE, readunlim=FALSE)
        modelName = att.get.nc(control, "NC_GLOBAL", "model_id")
        fConc= att.get.nc(control, "NC_GLOBAL","experiment_id")
        calendar <- att.get.nc(control, "time", "calendar")
        baseDate <- as.character(att.get.nc(control, "time", "units"))
        baseDate <- gsub("days since ","",baseDate)
        ...
        df$BaseDay[which(df$Model == model & df$Variable == varName & df$Experiment == experiment)] <- baseDay
        df$BaseMonth[which(df$Model == model & df$Variable == varName & df$Experiment == experiment)] <- baseMonth
        df$BaseYear[which(df$Model == model & df$Variable == varName & df$Experiment == experiment)] <- baseYear
        df3$BaseDay <- baseDay
        ...
        if(experiment == "Historical")
        {
          ...
          rDate = c(month = 1, day = 1, year = minGCMYear)
          timeIdxs = var.get.nc(control, "time")
        ...
        latmat = var.get.nc(control, "lat")
        lonmat = var.get.nc(control, "lon")
        ...
        # finds the indexes of the grid around the bounding box:
        ...
        {
          LeapFix <- "Yes"
          df$LeapFix[which(df$Model == model & df$Variable == varName & df$Experiment == experiment)] <- "Yes"
        } else {
          LeapFix <- "No"
          df$LeapFix[which(df$Model == model & df$Variable == varName & df$Experiment == experiment)] <- "No"
        }
        
        startTime = 1
        
        st = c(lonMin_idx,latMin_idx,startTime)        # lon, lat, time
        ct = c(lonMax_idx-lonMin_idx+1,latMax_idx-latMin_idx+1,tCount)
        data = var.get.nc(control, as.character(variableName), st, ct)
        
        # chooses one pixel and collects the data: FIX THIS!!! which pixel??
        px1 = 1
        py1 = 1
        
        # fixes a funny behavioir of the var.get.nc function: trims the dimensions of the results if the count of the dimension = 1
        if (length(which(ct>1)) == 1){ 
          var_val = data                            
        } else if (length(which(ct>1)) == 2) {
          var_val = data[px1,]
        } else {
          var_val = data[px1,py1,]
        }
        ...
        if(LeapFix == "Yes")
        {
          #if (fName == fList[1] & experiment == "Historical")
          #{
          #  leap_ind_add <- 0
          #} else if(fName == fList[1] & experiment != "Historical")
          #{
          #  leap_ind_add <- sum(leap.year(df3$BaseYear:minGCMYear))
          #}
          leap_ind_add <- 0
        ...
        if (as.character(variableName) == "pr"){
          ...
          results_daily_l$var_val[a] = NaN
        }
        
        results_daily = rbind(results_daily,results_daily_l) 
        print(fName)
  ...
  #---------------------------------------------------------
  # PART 2. READS OBSERVATIONS OF THE CORRESPONDING PIXEL
  #----------------------------------------------------------
  oPath = paste(main_folder,obs_files,sep="")
  # Reads all historic records from a CSV file:
  VAR_day = read.csv(oPath[1]) 
  VAR_month = read.csv(oPath[2])     # csv file with the variable pr, ts, etc
  ...
  catalog = read.csv(oPath[3],header=TRUE, sep=",")
    ...
    # SET THE BOUNDING OF THE PIXEL IN DEGREES:
  ...
  # -----------------------------------
  # saves dataframes with the results:
  ...
  plot(stages_d_x,stages_d_y,col = "blue" ,pch = 16, cex = 0.75,asp=1, 
       xlim=c(lonMin, lonMax), ylim=c(min(latMin, boundingBox["latMin"]), max(latMax, boundingBox["latMax"])*1.01),xlab="Lon", ylab="Lat")
  ...
  for (i in 1:dim(t(fFolders))[2]) {
    plot(ensemble_year$time_year,ensemble_year[,2],type="l", xlim = c(min(ensemble_year$time_year,data_y$Year[ay]), max(ensemble_year$time_year,data_y$Year[ay])), ylim=c(min(ensemble_year[,2][is.finite(ensemble_year[,2])], data_y$avg_value[ay][is.finite(data_y$avg_value[ay])]),1.01*max(ensemble_year[,2][is.finite(ensemble_year[,2])], data_y$avg_value[ay][is.finite(data_y$avg_value[ay])])),col = pallete[i],xlab="Year", ylab=variableLabel)
  }
...
#-----------------------------------------------
# III. FUNCTION TO PRODUCE STATS OF A GIVEN MODEL 
#-----------------------------------------------
compare_GCM = function(refDate = c(month = 1, day = 1, year = 1850),
  ...
  ensemble_day_f <- ensemble_day
  ensemble_day_f$month_idx = (ensemble_day$time_year - refDate[3]) * 12 + ensemble_day_f$time_month
  
  fName = paste(fPath,paste(modelName,fConc,varName,"ensemble_year.Rda",sep="_"),sep="")
  load(fName,verbose=TRUE)   
  ensemble_year_f <- ensemble_year
  
  fName = paste(fPath,paste(modelName,fConc,varName,"ensemble_month.Rda",sep="_"),sep="")
  load(fName,verbose=TRUE)   
  ensemble_month_f <- ensemble_month
  ...
  if(alignHistYears == TRUE)
  {
    ...
    ensemble_day <- ensemble_day[ensemble_day$time_year >= minObsYear,]
    ensemble_month <- ensemble_month[ensemble_month$time_year >= minObsYear,]
    ensemble_year <- ensemble_year[ensemble_year$time_year >= minObsYear,]
  }
  ...
  #----------------------------------------------------------------------
  # Calculates anomalies with respecto to refPeriod starting in Ref Year and ending now
  #----------------------------------------------------------------------
  
  aym = which((ensemble_year$time_year >= minObsYear)) 
  gcm_avg = mean(ensemble_year$pr_E1[aym],na.rm=TRUE)
  
  anm = which(is.na(ensemble_year$pr_E1))                    # completes missing data with the mean
  ensemble_year$pr_E1[anm] = gcm_avg
  
  ensemble_year$anomaly <- ensemble_year$pr_E1 - gcm_avg     # historic anomaly
  ...
  maxy <- 1.03*max(ensemble_month$pr_E1[is.finite(ensemble_month$pr_E1)], data_m$avg_value[is.finite(data_m$avg_value)], ensemble_month_f$pr_E1[is.finite(ensemble_month_f$pr_E1)])
  ...
  boxplot(pr_E1~time_month,data=ensemble_month, main=modelName, 
          xlab="Month", ylab="Precipitation (mm)", ylim = c(0,maxy), col="green3", boxwex  = 0.2, at = 1:12 - 0.0,yaxs    = "i" )   # historic model
  
  boxplot(avg_value[ay]~Month[ay],data=data_m, 
          xlab="Month",names = nam, ylab="Precipitation (mm)",col="cyan",  boxwex  = 0.2, at = 1:12 - 0.3,add=TRUE)  
  ...
  par(xpd = TRUE)
  legend(x="top", legend = c(paste0("Observed ", minObsYear, "-", max(data_m$Year)), paste0("GCM ", min(ensemble_month$time_year), "-",max(ensemble_year$time_year)), paste0("GCM ",min(ensemble_year_f$time_year), "-",maxGCMYear)),fill= c("cyan","green3", "orange" ), horiz = T, inset = c(0,-0.12), bty = 'n', text.width=c(1.5,1,2))
  ...
  sp_gcm = spectrum(ensemble_year$pr_E1,plot=F)
  ...
  tmin = minObsYear
  tmax = maxGCMYear
  pmin = min(ensemble_year$anomaly,ensemble_year_f$anomaly,data_y$anomaly[ayo], na.rm = T)
  pmax = max(ensemble_year$anomaly,ensemble_year_f$anomaly,data_y$anomaly[ayo], na.rm = T)
  
  ayo = which(data_y$Year >= minObsYear)
  
  plot(ensemble_year$time_year,ensemble_year$anomaly,type="l",col="green3",xlim=c(tmin, tmax), ylim=c(pmin, pmax),xlab="Year", ylab="Precipitation - Avg. [mm]")
  par(new=T)
  plot(ensemble_year_f$time_year,ensemble_year_f$anomaly,type="l",col="orange",xlim=c(tmin, tmax), ylim=c(pmin, pmax),xlab="", ylab="")
  par(new=T)
  plot(data_y$Year[ayo],data_y$anomaly[ayo],type="l",col="blue",xlim=c(tmin, tmax), ylim=c(pmin, pmax),xlab="", ylab="")
  ...
  legend( x="top", 
          ...
          col=c("blue","green3","orange"), lwd=1.5, lty=c(1,1,1), 
  ...
  for (i in 1:ceiling((max(ensemble_year$time_year) - minObsYear)/30)){
    bYear = minObsYear + (i - 1) * 30
    eYear = minObsYear  + (i) * 30
    
    aym_b = which(ensemble_day$time_year >= bYear)
    aym_b = aym_b[1]
    aym_e = which(ensemble_day$time_year < eYear)
    aym_e = tail(aym_e, n=1)
    
    aym = aym_b:aym_e
    
    wf_model[i] = wet_fraction(ensemble_day$pr_E1[aym],1)
    
    spellLengths_model = wet_dry_spell_length(ensemble_day$pr_E1[aym],1)
    wet_spell_model[i] = spellLengths_model[6]
    dry_spell_model[i] = spellLengths_model[5]
    
    p_05_model[i] = percentile(ensemble_day$pr_E1[aym],0.05)
    b_model[i] = bias(data_d$avg_value[ayo],ensemble_day$pr_E1[aym])
    mean_model[i] = mean(ensemble_day$pr_E1[aym],na.rm=TRUE)
...
knn_bootstrap = function(refDate = c(month = 1, day = 1, year = 1850),
  ...
  fName = paste(fPath,paste(modelName,"historical",varName,"ensemble_day.Rda",sep="_"),sep="")
  load(fName,verbose=TRUE) 
  ensemble_day$julian_day <- julian(ensemble_day$time_month,ensemble_day$time_day,1850,origin = c(month = 1, day = 1, 1850))
  ...
  if(alignHistYears == TRUE)
  {
    maxObsYear <- min(max(data_d$Year), max(ensemble_day$time_year))
  } else {
  ...
  ensemble_day <- ensemble_day[ensemble_day$time_year >= minObsYear & ensemble_day$time_year <= maxObsYear,]
  ensemble_month <- ensemble_month[ensemble_month$time_year >= minObsYear & ensemble_month$time_year <= maxObsYear,]
  ensemble_year <- ensemble_year[ensemble_year$time_year >= minObsYear & ensemble_year$time_year <= maxObsYear,]
  ...
  if (JPmode == "Yearly")
  {
    ...
    ensemble_day_filt <- ensemble_day
  ...
  for (y in 1:nYears) {
    ...
    for (i in 0:364) {   # 364 days
      ...
      if (i==0) {   # required to initialize the synthetic generator. is based on the marginal distribution
        ...
        dffilt <- df[df$Model == model & df$Experiment == "Historical" & df$Variable == varName,]
...
#------------------------------------------------------------------------------------------#
  ...
  if(HistRepro != TRUE)
  {
    ...
    exThreshold <- 2
    repeat {
      GPDbs <- try(gpd(sort(bs_result$bs_value),threshold=exThreshold),silent = TRUE)
    ...
    save(bs_excorr,file=paste(main_folder,results_folder,"/",paste(modelName,"bs", "pr",futures,"day_ExtremesCorrected_key.Rda",sep="_"),sep=""))
  }
  ...
  pdf(file=paste0(main_folder,results_folder,modelName,'_',fConc,"_",varName,'_Bootstrap_Boxplots.pdf',sep=""), height=8, width=14, onefile=TRUE, family='Helvetica', paper="a4r", pointsize=12) 
  ...
  #--------------------------------------------------------
  pdf(file=paste0(main_folder,results_folder,modelName,'_',fConc,"_",varName,"_Threshold_Comp_Plots.pdf"), paper = 'a4r', height=8.5, width=11)
  ...
  pdf(file=paste0(main_folder,results_folder,modelName,'_',fConc,"_",varName,"_JP_Monthly_plots.pdf"))
...
heatSignal = function(refDate = c(month = 1, day = 1, year = 1850),
  ...
  df_heat_signal_WEAP = df_heat_signal[,!(names(df_heat_signal) %in% drops)]
  write.table(df_heat_signal_WEAP,paste(fPath,paste(modelName,fConc,"TS_HeatSignal_WEAP.csv",sep="_"),sep=""), sep=",",row.names=FALSE, na = "-9999")
  
  #-------------------------------------------------------------------------
  # PLOTS OF TEMPERATURE COMPARISON OBS, GCM HIST, GCM FUTURE
  #-------------------------------------------------------------------------
  
  pdf(file=paste(main_folder,results_folder,"/",modelName,'_',fConc,"_",varName,'_Temp_Med_Comp_BoxPlots.pdf',sep=""), height=8.5, width=11, onefile=TRUE, family='Helvetica', paper="a4r", pointsize=12) 
...
sinteticSeries = function(refDate = c(month = 1, day = 1, year = 1850),
  ...
  extrcorr_regiones <- c("Alto Mag ExtrCorr" , "Medio Mag ExtrCorr" , "Antioquia ExtrCorr" , "Sogamoso ExtrCorr" , "Bajo Mag ExtrCorr" , "Caribe ExtrCorr")
  fullCatalogFilePr = paste0("/OBSERVATIONS/Magdalena-Cauca/","SERIES_pr_DIARIO_v4.Rda")
  fullCatalogFileTemp = "/OBSERVATIONS/Magdalena-Cauca/SERIES_tas_DIARIO_v4.Rda"
  ...
  fName = paste(fPath,paste(modelName,"historical","tas","ensemble_day.Rda",sep="_"),sep="")
  load(fName) # ensemble_day
  ...
  fName = paste(main_folder,results_folder,"/",paste(modelName,"heatSignal_.Rda",sep="_"),sep="")
  ...
  if(fullObsCatalog == TRUE)
  {
    ...
    bs_local_WEAPformat_ALL <- bs_pr_local_data_ALL[,!(names(bs_pr_local_data_ALL) %in% drops)]
    
    #adds the date strings and rounds the numbers for a neater file
    write.table(cbind(bs_local_WEAPformat_ALL$bs_timeStamp,round(bs_local_WEAPformat_ALL[,!(names(bs_local_WEAPformat_ALL) %in% "bs_timeStamp")],2)),
                paste(fPath,paste(modelName,futures,"pr_Sintetic_Series_ALL_Stations",expNumber,"WEAP.csv",sep="_"),sep=""), 
    ...
    tas_local_WEAPformat_ALL = tas_local_data_ALL[,!(names(tas_local_data_ALL) %in% drops)]
    write.table(cbind(tas_local_WEAPformat_ALL$bs_timeStamp,round(tas_local_WEAPformat_ALL[,!(names(tas_local_WEAPformat_ALL) %in% "bs_timeStamp")],1)),
                paste(fPath,paste(modelName,fConc,"tas_Sintetic_Series_ALL_stations",expNumber,"WEAP.csv",sep="_"),sep=""), 
                sep=",",row.names=FALSE,col.names=FALSE, na = "-9999",quote=FALSE)
  ...
  drops <- c("simYear","simMonth","simDay","Year.x","Month.x","Day.x","bs_value",  "bs_wet_state",	"bs_brick_size","index","TimeStamp","Year.y","Month.y","Day.y")
  bs_local_WEAPformat = bs_pr_local_data[,!(names(bs_pr_local_data) %in% drops)]
  
  #adds the date strings and rounds the numbers for a neater file
  write.table(cbind(bs_local_WEAPformat$bs_timeStamp,round(bs_local_WEAPformat[,!(names(bs_local_WEAPformat) %in% "bs_timeStamp")],2)),
              paste(fPath,paste(modelName,futures,"pr_Sintetic_Series",expNumber,"WEAP.csv",sep="_"),sep=""), 
              sep=",",row.names=FALSE,col.names=FALSE,na = "-9999",quote=FALSE)
  
  tas_local_WEAPformat = tas_local_data[,!(names(tas_local_data) %in% drops)]
  write.table(cbind(tas_local_WEAPformat$bs_timeStamp,round(tas_local_WEAPformat[,!(names(tas_local_WEAPformat) %in% "bs_timeStamp")],1)),
              paste(fPath,paste(modelName,fConc,"tas_Sintetic_Series",expNumber,"WEAP.csv",sep="_"),sep=""), 
              sep=",",row.names=FALSE,col.names=FALSE, na = "-9999",quote=FALSE)
...
extremePR_GPD = function(modelName = "MPI-ESM-MR",
  ...
  pdf(file=paste(main_folder,results_folder,"/",modelName,'_',futures,'_Extremes.pdf',sep=""), 
